...
import { Bell, Store, ShoppingCart, AlertTriangle, CheckCheck, Trash2, X } from 'lucide-react';
import { SkeletonCard } from '../../components/ui/Skeleton';
import { useNotifications } from '../../hooks/useNotifications';

const typeConfig = {
  'store.approved':  { icon: Store,         color: 'var(--success-dark)', bg: 'var(--success-light)' },
  ...
  'order.new':       { icon: ShoppingCart,  color: 'var(--blue-700)',     bg: 'var(--blue-50)' },
    ...
    <div style={{ maxWidth:720 }}>
      ...
      {notifications.length === 0 && (
        <div style={{ textAlign:'center', padding:'var(--sp-12)', color:'var(--ink-400)', background:'var(--white)', border:'1px solid var(--ink-200)', borderRadius:16 }}>
          ...
          <p style={{ fontSize:13 }}>Platform alerts will appear here.</p>
        </div>
      )}
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:'var(--sp-2)' }}>
        <AnimatePresence initial={false}>
          {notifications.map((n, i) => {
            ...
            const isUrgent = !isRead && (n.type.includes('reject') || n.type.includes('suspend'));
            return (
              <motion.div key={n.id} layout
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3, flexWrap:'wrap' }}>
                    ...
                    {isUrgent && <span style={{ fontSize:10, fontWeight:700, color:'var(--danger)', background:'#FFF1F2', border:'1px solid #FECACA', padding:'1px 7px', borderRadius:9999 }}>ACTION NEEDED</span>}
                    {!isRead && !isUrgent && <span style={{ width:7, height:7, borderRadius:'50%', background:'var(--blue-700)', display:'inline-block' }} />}
                  </div>
