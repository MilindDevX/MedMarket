...
const typeConfig = {
  ...
  'order.accept':   { icon: CheckCircle,   color: 'var(--success-dark)', bg: 'var(--success-light)' },
  'order.pack':     { icon: ShoppingBag,   color: 'var(--blue-700)',     bg: 'var(--blue-50)' },
  'order.dispatch': { icon: CheckCircle,   color: 'var(--success-dark)', bg: 'var(--success-light)' },
  'order.deliver':  { icon: CheckCircle,   color: 'var(--success-dark)', bg: 'var(--success-light)' },
  'order.reject':   { icon: AlertTriangle, color: 'var(--warning-dark)', bg: 'var(--warning-light)' },
  'order.cancel':   { icon: AlertTriangle, color: 'var(--warning-dark)', bg: 'var(--warning-light)' },
...
export default function ConsumerNotifications() {
  ...
  if (loading) return <div style={{ maxWidth:640, margin:'0 auto', padding:'var(--sp-6) var(--sp-5)' }}><SkeletonCard lines={5} /></div>;
    ...
    <div style={{ maxWidth:640, margin:'0 auto', padding:'var(--sp-6) var(--sp-5)' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'var(--sp-6)', flexWrap:'wrap', gap:'var(--sp-3)' }}>
        <div>
          ...
          <p style={{ fontSize:13, color:'var(--ink-500)', marginTop:4 }}>{notifications.length} notifications · {unreadCount} unread</p>
        </div>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:'var(--sp-2)' }}>
        <AnimatePresence initial={false}>
          {notifications.map((n, i) => {
            ...
            const isUrgent = !isRead && (n.type === 'order.reject' || n.type === 'order.cancelled');
            return (
              <motion.div key={n.id} layout
                ...
                style={{ background: isRead ? 'var(--ink-50)' : isUrgent ? '#FFF5F5' : 'var(--white)', border:`1px solid ${isUrgent && !isRead ? '#FECACA' : 'var(--ink-200)'}`, borderRadius:14, padding:'var(--sp-4)', display:'flex', gap:'var(--sp-3)', cursor: isRead ? 'default' : 'pointer', position:'relative' }}>
