...
import { AlertTriangle, XCircle, Clock, X, Check, Tag } from 'lucide-react';
...
export default function ExpiryAlerts() {
  ...
  const [discountTarget, setDiscountTarget] = useState(null); // inventory item
  ...
  const alertItems = inventory.filter(i => i.exp_date && getStatus(i) !== 'ok');

  const openDiscount = (item) => {
    setDiscountTarget(item);
    // Pre-fill with 10% off current selling price
  ...
  return (
    <div>
      ...
      {groups.map(({ key, icon:Icon, label, color, bg, border }) => {
        const items = alertItems.filter(i => getStatus(i) === key);
        if (!items.length) return null;
        return (
          <div key={key} style={{ marginBottom:'var(--sp-6)' }}>
            <div style={{ display:'flex', alignItems:'center', gap:'var(--sp-2)', marginBottom:'var(--sp-3)' }}>
              <Icon size={16} strokeWidth={2} style={{ color }} />
              <h2 style={{ fontSize:15, fontWeight:700, color:'#1C1C1A' }}>{label}</h2>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:'var(--sp-3)' }}>
              {items.map((item, i) => (
                <motion.div key={item.id}
                  ...
                  <div style={{ fontSize:15, fontWeight:700, color:'#1C1C1A', marginBottom:4 }}>{item.medicine?.name}</div>
                  <div style={{ fontSize:12, color:'#5A5F6B', fontFamily:'monospace', marginBottom:'var(--sp-3)' }}>{item.batch_number}</div>
                  <div style={{ display:'flex', justifyContent:'space-between', fontSize:13, color:'#3D3D3A', marginBottom:4 }}>
                    <span>EXP: <strong>{item.exp_date?.split('T')[0]}</strong></span>
                    <span><strong>{item.quantity}</strong> units</span>
                  </div>
                  <div style={{ display:'flex', justifyContent:'space-between', fontSize:13, color:'#3D3D3A', marginBottom:'var(--sp-3)' }}>
                    <span>MRP: ₹{item.medicine?.mrp}</span>
                    <span>Selling: <strong>₹{item.selling_price}</strong></span>
                  </div>
                  <div style={{ display:'flex', gap:'var(--sp-2)' }}>
                    {key !== 'expired' && (
                      <button onClick={() => openDiscount(item)}
                        style={{ flex:1, padding:'7px', background:'var(--green-700)', color:'#FFFFFF', border:'none', borderRadius:6, fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:'var(--font-body)', display:'flex', alignItems:'center', justifyContent:'center', gap:5 }}>
                        <Tag size={12} strokeWidth={2.5} /> Apply Discount
                      </button>
                    )}
                    <button onClick={() => setRemoveTarget(item)}
                      style={{ flex:1, padding:'7px', background:'#FFFFFF', color:'#991B1B', border:'1px solid #FECACA', borderRadius:6, fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:'var(--font-body)' }}>
                      Remove Stock
                    </button>
                  </div>
                </motion.div>
      ...
      <AnimatePresence>
        {discountTarget && (
          <motion.div
            ...
            <motion.div
              style={{ background:'#FFFFFF', borderRadius:20, width:'100%', maxWidth:400, padding:'var(--sp-6)', boxShadow:'0 20px 60px rgba(0,0,0,0.2)' }}
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'var(--sp-4)' }}>
                ...
                <button onClick={() => setDiscountTarget(null)} style={{ padding:4, color:'var(--ink-400)', borderRadius:6 }}><X size={16}/></button>
              </div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'var(--sp-3)', marginBottom:'var(--sp-4)', fontSize:13 }}>
              ...
              <div style={{ display:'flex', gap:'var(--sp-3)' }}>
                <button onClick={() => setDiscountTarget(null)}
                  style={{ flex:1, padding:11, background:'#FFFFFF', color:'var(--ink-700)', border:'1px solid var(--ink-200)', borderRadius:10, fontSize:14, fontWeight:600, cursor:'pointer', fontFamily:'var(--font-body)' }}>
                  Cancel
                </button>
                <button onClick={handleApplyDiscount} disabled={discounting}
                  style={{ flex:1, padding:11, background:'var(--green-700)', color:'#FFFFFF', border:'none', borderRadius:10, fontSize:14, fontWeight:700, cursor: discounting ? 'not-allowed' : 'pointer', fontFamily:'var(--font-body)', display:'flex', alignItems:'center', justifyContent:'center', gap:6, opacity: discounting ? 0.7 : 1 }}>
            ...
            <motion.div
              ...
              <div style={{ display:'flex', gap:'var(--sp-3)' }}>
                ...
                <button onClick={handleRemove} disabled={removing}
                  style={{ flex:1, padding:11, background:'var(--danger)', color:'#FFFFFF', border:'none', borderRadius:10, fontSize:14, fontWeight:700, cursor: removing ? 'not-allowed' : 'pointer', fontFamily:'var(--font-body)', opacity: removing ? 0.7 : 1 }}>
