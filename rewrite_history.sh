...
git commit -m "chore: initial project setup and tooling" --allow-empty
...
git commit -m "chore(db): initialize Prisma and database schema" --allow-empty
...
git commit -m "chore: core express server setup" --allow-empty
...
git commit -m "chore(frontend): setup global styles and design tokens" --allow-empty
...
git commit -m "feat(auth): implement core JWT authentication middleware" --allow-empty
...
git commit -m "feat(auth): authentication API" --allow-empty
...
git commit -m "test(auth): add unit tests for authentication endpoints" --allow-empty

# 12. feat(auth): add rate limiting on auth endpoints
git add backend/src/middleware/rateLimit.middleware.ts 2>/dev/null || true
git commit -m "feat(auth): add rate limiting on auth endpoints" --allow-empty
...
git commit -m "feat(auth): implement Google OAuth login flow" --allow-empty
...
git commit -m "feat(core): error handling and validation middleware" --allow-empty
...
git commit -m "feat(core): cloudinary integration for file uploads" --allow-empty
...
git commit -m "feat(consumer): address book management API" --allow-empty
...
git commit -m "feat(store): store profile and discovery API" --allow-empty
...
git commit -m "feat(medicine): medicine catalog API" --allow-empty
...
git commit -m "feat(inventory): pharmacy inventory management API" --allow-empty
...
git commit -m "test(inventory): add unit tests for OTC and MRP constraints" --allow-empty
...
git commit -m "feat(order): order management API" --allow-empty
...
git commit -m "test(order): add unit tests for order state machine" --allow-empty
...
git commit -m "feat(admin): pharmacy approval and rejection endpoints" --allow-empty
...
git commit -m "feat(admin): master medicine catalogue management" --allow-empty
...
git commit -m "feat(admin): platform KPIs and settings endpoints" --allow-empty
...
git commit -m "test(settings): add unit tests for platform settings" --allow-empty
...
git commit -m "feat(notification): in-app notifications API" --allow-empty
...
git commit -m "feat(frontend): initial React setup and router configuration" --allow-empty
...
git commit -m "test(frontend): add setup configuration for frontend tests" --allow-empty
...
git commit -m "feat(store): add authentication and user zustand stores" --allow-empty
...
git commit -m "feat(store): add cart, order, and inventory state stores" --allow-empty
...
git commit -m "test(store): add unit tests for cart and location stores" --allow-empty
...
git commit -m "feat(api): utility functions and Axios API client" --allow-empty

# 34. test(api): add unit tests for API client
git add frontend/src/__tests__/api.test.js 2>/dev/null || true
git commit -m "test(api): add unit tests for API client" --allow-empty
...
git commit -m "feat(hooks): add core data fetching hooks" --allow-empty
...
git commit -m "feat(ui): public and authentication layouts" --allow-empty
...
git commit -m "feat(ui): consumer portal layouts and navigation" --allow-empty
...
git commit -m "feat(ui): admin and pharmacy dashboard layouts" --allow-empty

# 39. feat(ui): extract reusable pagination component
git add frontend/src/components/ui/Pagination.jsx 2>/dev/null || true
git commit -m "feat(ui): extract reusable pagination component" --allow-empty
...
git commit -m "feat(public): landing page and static content" --allow-empty
...
git commit -m "feat(public): user and pharmacy authentication pages" --allow-empty
...
git commit -m "feat(consumer): home page and local store discovery" --allow-empty
...
git commit -m "feat(consumer): medicine browsing and detail views" --allow-empty
...
git commit -m "feat(consumer): shopping cart and checkout flow" --allow-empty
...
git commit -m "feat(consumer): order tracking and history" --allow-empty
...
git commit -m "feat(consumer): user profile and notifications" --allow-empty

# 47. feat(pharmacy): pharmacy registration flow
git add frontend/src/pages/pharmacy/PharmacyRegister* 2>/dev/null || true
git commit -m "feat(pharmacy): pharmacy registration flow" --allow-empty
...
git commit -m "feat(pharmacy): pharmacy dashboard and analytics hooks" --allow-empty
...
git commit -m "feat(pharmacy): inventory management and expiry alerts" --allow-empty
...
git commit -m "feat(pharmacy): incoming order management" --allow-empty
...
git commit -m "feat(pharmacy): pharmacy complaints and profile settings" --allow-empty
...
git commit -m "feat(admin): admin dashboard and platform analytics" --allow-empty
...
git commit -m "feat(admin): pharmacy application review interface" --allow-empty

# 54. feat(admin): master medicine catalogue interface
git add frontend/src/pages/admin/AdminMedicines* 2>/dev/null || true
git commit -m "feat(admin): master medicine catalogue interface" --allow-empty
...
git commit -m "feat(admin): platform settings and configuration interface" --allow-empty

# 59. chore: add frontend static assets
git add frontend/public/ frontend/src/assets/ 2>/dev/null || true
git commit -m "chore: add frontend static assets" --allow-empty

# 60. Final catch-all for any files that were accidentally missed
git add .
git commit -m "chore: ensure all files are included" --allow-empty

# Cleanup empty commits
git filter-branch -f --commit-filter '
    if [ -z "$(git diff-tree -p $commit)" ]; then
        skip_commit "$@";
    else
        git commit-tree "$@";
    fi' HEAD
